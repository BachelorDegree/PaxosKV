...
public:
  ...
  virtual int Execute(uint32_t iGroupIndex, uint64_t instance_id, const std::string &strChosenValue)
  {
    ...
    this->m_pClients->Set(iGroupIndex, "CPID", s_a);
    return 0;
  }
  int DoPut(uint32_t iGroupIndex, const kvsm::Request &oReq, kvsm::Response &oResp)
  {
    int iRet = m_pClients->Set(iGroupIndex, oReq.key(), oReq.value());
    if (iRet != 0)
    {
  ...
  {
    int iRet = m_pClients->Del(iGroupIndex, oReq.key());
    if (iRet != 0)
    {
  ...
  {
    std::string value;
    int iRet = this->m_pClients->Get(iGroupIndex, "CPID", value);
